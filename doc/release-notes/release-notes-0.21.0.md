...
Bitcoin Core version 0.21.0 is now available from:
  ...
  <https://bbcoincore.org/bin/bbcoin-core-0.21.0/>
...
- This release adds support for Tor version 3 hidden services, and rumoring them
  ...
  [BIP155](https://github.com/bbcoin/bips/blob/master/bip-0155.mediawiki).
...
- This release adds support for serving
  [BIP157](https://github.com/bbcoin/bips/blob/master/bip-0157.mediawiki) compact
...
- This release adds support for signets
  ([BIP325](https://github.com/bbcoin/bips/blob/master/bip-0325.mediawiki)) in
...
- This release implements
  [BIP339](https://github.com/bbcoin/bips/blob/master/bip-0339.mediawiki)
...
- This release implements the proposed Taproot consensus rules
  ([BIP341](https://github.com/bbcoin/bips/blob/master/bip-0341.mediawiki) and
  [BIP342](https://github.com/bbcoin/bips/blob/master/bip-0342.mediawiki)),
...
- A new `bbcoin-cli -netinfo` command provides a network peer connections
...
- A new `bbcoin-cli -generate` command, equivalent to RPC `generatenewaddress`
  followed by `generatetoaddress`, can generate blocks for command line testing
  purposes. This is a client-side version of the former `generate` RPC. See the
  help for details. (#19133)

- The `bbcoin-cli -getinfo` command now displays the wallet name and balance for
  each of the loaded wallets when more than one is loaded (e.g. in multiwallet
  mode) and a wallet is not specified with `-rpcwallet`. (#18594)

- The `connections` field of `bbcoin-cli -getinfo` is now expanded to return a JSON
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `bbcoin-wallet` tool which performs the salvage
...
`bbcoin.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
Bitcoin Core will now just log warnings instead of creating new wallets with
new keys and addresses like previous releases did.

New wallets can be created through the GUI (which has a more prominent create
wallet option), through the `bbcoin-cli createwallet` or `bbcoin-wallet
...
Bugs and missing functionality can be reported to the [issue tracker](https://github.com/bbcoin/bbcoin/issues).
...
- A new `fee_rate` parameter/option denominated in satoshis per vbyte (sat/vB)
  ...
  `walletcreatefundedpsbt` still exists for setting a fee rate in BBCO per 1,000
  vbytes (BBCO/kvB), but it is expected to be deprecated soon to avoid
  confusion. For these RPCs, the fee rate error message is updated from BBCO/kB
  to sat/vB and the help documentation in BBCO/kB is updated to BBCO/kvB. The
  `send` and `sendtoaddress` RPC examples are updated to aid users in creating
  transactions with explicit fee rates. (#20305, #11413)

- The `bumpfee` RPC `fee_rate` option is changed from BBCO/kvB to sat/vB and the
  ...
  difference between BBCO/kvB and sat/vB units means that a transaction with a
  fee rate mistakenly calculated in BBCO/kvB rather than sat/vB should raise an
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  ...
  `\<datadir\>/settings.json` and augments any command line or `bbcoin.conf`
...
- #19284 Add seed.bbcoin.wiz.biz to DNS seeds (wiz)
...
- #20198 Show name, format and if uses descriptors in bbcoin-wallet tool (jonasschnelli)
...
- #19133 rpc, cli, test: add bbcoin-cli -generate command (jonatack)
...
- #20055 Set HTTP Content-Type in bbcoin-cli (laanwj)
...
### Build system
- #18504 Drop bbcoin-tx and bbcoin-wallet dependencies on libevent (ryanofsky)
...
- #18653 add coverage for bbcoin-cli -rpcwait (jonatack)
...
- #19061 Add link to Visual Studio build readme (maitrebbcoin)
...
- #18817 Document differences in bbcoind and bbcoin-qt locale handling (practicalswift)
