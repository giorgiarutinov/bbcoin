...
Bitcoin Core version 0.20.0 is now available from:
  ...
  <https://bbcoincore.org/bin/bbcoin-core-0.20.0/>
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/bbcoin/bbcoin/issues/16599),
  [PR #16702](https://github.com/bbcoin/bbcoin/pull/16702), and the
  `bbcoind help` for more information.  This option is experimental and
...
- A "known bug" [announced](https://bbcoincore.org/en/releases/0.18.0/#wallet-gui)
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  program such as [HWI](https://github.com/bbcoin-core/HWI) for
...
- Bitcoin Core's automatically-generated source code documentation is
  now available at https://doxygen.bbcoincore.org.  (#17596)
...
- The `bbcoin-cli` utility used with the `-getinfo` parameter now
...
- The `bbcoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `bbcoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_bbcoin-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_bbcoin` binary (hebasto)
...
- #18702 Fix ASLR for bbcoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate bbcoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest bbcoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_bbcoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (bbcoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone bbcoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #17119 Fix broken bbcoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from bbcoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.bbcoincore.org (laanwj)
...
- #17648 Rename wallet-tool references to bbcoin-wallet (hel-o)
...
- #18341 Replace remaining literal BBCO with `CURRENCY_UNIT` (domob1812)
...
- #16115 On bbcoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from bbcoin-cli -getinfo (malevolent)
