Bitcoin Core version 0.19.0.1 is now available from:
  ...
  <https://bbcoincore.org/bin/bbcoin-core-0.19.0.1/>
...
- [Reduce memory](https://github.com/bbcoin/bbcoin/blob/master/doc/reduce-memory.md)
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `bbcoind -help` output for these two options for
...
- The `testnet` field in `bbcoin-cli -getinfo` has been renamed to
...
- When creating a transaction with a fee above `-maxtxfee` (default 0.1
  BBCO), the RPC commands `walletcreatefundedpsbt` and
  `fundrawtransaction` will now fail instead of rounding down the fee.
  Be aware that the `feeRate` argument is specified in BBCO per 1,000
...
- #15639 bbcoin-wallet tool: Drop libbbcoin_server.a dependency (ryanofsky)
...
- #16521 Use the default maxfeerate value as BBCO/kB (Remagpie)
...
- #16366 init: Use InitError for all errors in bbcoind/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "bbcoin-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of bbcoind build (NicolasDorier)
...
- #16441 remove qt libjpeg check from bbcoin_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_bbcoin to auto generated project files (sipsorcery)
...
- #15352 Reduce noise level in test_bbcoin output (practicalswift)
...
- #15896 feature_filelock, interface_bbcoin_cli: Use PACKAGE_NAME in messages rather than hardcoding Bitcoin Core (luke-jr)
...
- #16277 Suppress output in test_bbcoin for expected errors (gertjaap)
...
- #16929 follow-up to rpc: default maxfeerate value as BBCO/kB (jonatack)
...
- #16489 log: harmonize bbcoind logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (bbcoinVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example bbcoin.conf (fanquake)
...
- #16080 build/doc: update bbcoin_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in bbcoin.conf (fanquake)
...
- #16621 add default bbcoin.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the bbcoin-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, bbcoin.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/bbcoind.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/bbcoin/bbcoin/).
