...
Bitcoin Core version 26.0 is now available from:
  ...
  <https://bbcoincore.org/bin/bbcoin-core-26.0/>
...
- Experimental support for the v2 transport protocol defined in
  [BIP324](https://github.com/bbcoin/bips/blob/master/bip-0324.mediawiki) was added.
...
- [Miniscript](https://bbcoin.sipa.be/miniscript/) expressions can now be used in Taproot descriptors for all RPCs working with descriptors. (#27255)

- `finalizepsbt` is now able to finalize a PSBT with inputs spending [Miniscript](https://bbcoin.sipa.be/miniscript/)-compatible Taproot leaves. (#27255)
...
- `bbcoind` and `bbcoin-qt` will now raise an error on startup
 if a datadir that is being used contains a bbcoin.conf file that
 will be ignored, which can happen when a datadir= line is used in
 a bbcoin.conf file. The error message is just a diagnostic intended
 ...
 the bbcoin.conf contained in it. (#27302)
...
- A new `bbcoinconsensus_verify_script_with_spent_outputs` function is available in libconsensus which optionally accepts the spent outputs of the transaction being verified.
- A new `bbcoinconsensus_SCRIPT_FLAGS_VERIFY_TAPROOT` flag is available in libconsensus that will verify scripts with the Taproot spending rules.
...
```sh
src/bbcoin-cli -named bumpfee txid fee_rate=100
```

instead of

```sh
src/bbcoin-cli -named bumpfee txid options='{"fee_rate": 100}'
```
...
- It's now possible to use [Miniscript](https://bbcoin.sipa.be/miniscript/) inside Taproot leaves for descriptor wallets. (#27255)
...
- Bash completion files have been renamed from `bbcoin*.bash-completion` to
  `bbcoin*.bash`. This means completions can be automatically loaded on demand
