...
example /usr/src/bbcoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\bbcoin`, for example:
```shell
cmake --install build --prefix /mnt/c/workspace/bbcoin
```
...
```shell
cmake --install build --prefix /mnt/c/workspace/bbcoin --strip
```
