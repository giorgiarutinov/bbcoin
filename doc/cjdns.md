...
CJDNS peers than Tor or IP ones. You can use `bbcoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/bbcoin/bbcoin/blob/master/doc/tor.md) for
details.
...
To see which CJDNS peers your node is connected to, use `bbcoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `bbcoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `bbcoin-cli help getnodeaddresses` for details.
