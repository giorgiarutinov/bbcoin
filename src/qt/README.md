...
- ClientModel is used by the main application `bbcoingui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a bbcoin:... link from a browser).
...
* `bbcoinstrings.cpp`: automatically generated
* `bbcoinunits.(h/cpp)`: BBCO / mBBCO / etc. handling
* `callback.h`
...
**Note:** Do not change `local/bbcoin_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `bbcoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `bbcoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `bbcoin-qt`
