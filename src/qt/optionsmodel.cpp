...
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // in later releases by https://github.com/bbcoin/bbcoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because bbcoin 22.x will not try to read these.
...
//! Convert enabled/size values to bbcoin -prune setting.
...
//! Get pruning enabled value to show in GUI from bbcoin -prune setting.
...
//! Get pruning size value to show in GUI from bbcoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    if (!settings.contains("DisplayBitcoinUnit")) {
        settings.setValue("DisplayBitcoinUnit", QVariant::fromValue(BitcoinUnit::BBCO));
    }
    QVariant unit = settings.value("DisplayBitcoinUnit");
    if (unit.canConvert<BitcoinUnit>()) {
        m_display_bbcoin_unit = unit.value<BitcoinUnit>();
    } else {
        m_display_bbcoin_unit = BitcoinUnit::BBCO;
        settings.setValue("DisplayBitcoinUnit", QVariant::fromValue(m_display_bbcoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if bbcoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_bbcoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<BitcoinUnit>() == m_display_bbcoin_unit) return;
    m_display_bbcoin_unit = new_unit.value<BitcoinUnit>();
    QSettings settings;
    settings.setValue("DisplayBitcoinUnit", QVariant::fromValue(m_display_bbcoin_unit));
    Q_EMIT displayUnitChanged(m_display_bbcoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/bbcoin/bbcoin/pull/8273
    ...
    // (https://github.com/bbcoin-core/gui/issues/567).
