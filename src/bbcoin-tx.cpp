...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " bbcoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The bbcoin-tx tool is used for creating and modifying bbcoin transactions.\n\n"
                "bbcoin-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded bbcoin transaction, or with \"-create [commands]\" to create a hex-encoded bbcoin transaction.\n"
                "\n"
                "Usage: bbcoin-tx [options] <hex-tx> [commands]\n"
                "or:    bbcoin-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded bbcoin transaction
