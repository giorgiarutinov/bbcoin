...
The bpftrace examples contain a relative path to the `bbcoind` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `bbcoind` binary. The paths in the examples can be changed, for
...
```
$ python3 contrib/tracing/p2p_monitor.py $(pidof bbcoind)
```
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py $(pidof bbcoind)
```
...
```
$ ./build/src/bbcoind -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py $(pidof bbcoind)
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py $(pidof bbcoind)
```
