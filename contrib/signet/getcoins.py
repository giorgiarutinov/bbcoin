...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing bbcoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='bbcoin-cli', help='bbcoin-cli command to use')
...
parser.add_argument('bbcoin_cli_args', nargs='*', help='Arguments to pass on to bbcoin-cli (default: -signet)')

args = parser.parse_args()

if args.bbcoin_cli_args == []:
    args.bbcoin_cli_args = ['-signet']


def bbcoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.bbcoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = bbcoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = bbcoin_cli(['getnewaddress', 'faucet', 'bech32'])
