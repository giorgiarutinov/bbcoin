...
BASE_SCRIPTS = [
    ...
    'interface_bbcoin_cli.py --legacy-wallet',
    'interface_bbcoin_cli.py --descriptors',
...
def main():
    ...
    enable_bbcoind = config["components"].getboolean("ENABLE_BITCOIND")

    if not enable_bbcoind:
...
def run_tests(*, test_list, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control, results_filepath=None):
    ...
    # Warn if bbcoind is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "bbcoind"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a bbcoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
    ...
    # a hard link or a copy on any platform. See https://github.com/bbcoin/bbcoin/pull/27561.
...
class RPCCoverage():
    ...
    commands per `bbcoin-cli help` (`rpc_interface.txt`).
